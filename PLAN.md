...
### Turnstile — боевые ключи ✅ 20.07.2026

Ассистент через дашборд Cloudflare владельца (с его явного «сделай сам»): создан виджет **`aiym`**, режим **Managed**, hostname `aiym.faizov-midat.workers.dev`. Sitekey `0x4AAAAAAD5SDaDG6jwN1y4T` → в `vars` (публичный по определению — уходит в demo.html); secret → `wrangler secret put TURNSTILE_SECRET_KEY`. Задеплоено.

**Приёмка (обе стороны, иначе доказательства нет):**
- **Гейт настоящий:** мусорный `turnstile_token:"x"`, который с тестовыми ключами ПРОХОДИЛ, теперь → `403 turnstile_required`; без токена → `403`.
- **Живые люди проходят:** реальный браузер на /demo — Managed решил челлендж **невидимо** (iframe нет, токен появился в скрытом `cf-turnstile-response`), сообщение ушло, Айым ответила ценами окрашивания. То есть демо не сломано.

⚠️ Побочный эффект для тестирования: curl-проверки `/chat` больше нельзя гонять с фиктивным токеном — нужен реальный браузер (либо временно вернуть тестовый sitekey). Это и есть признак, что защита включена.

**Осталось владельцу:** `wa.me` для лендинга (номер не выдумывал); шаги этапа 8 (триал Bitrix24, пост в @itmankz, рассылка) — материалы в `launch/`.

### «Оживление» продукта: боевые лимиты, онбординг, мониторинг ✅ 20.07.2026

По просьбе «сделай всё, чтобы ожил» — три технических блока (продажи и покупку платного ElevenLabs не трогал: первое за владельцем, второе бессмысленно без клиента).

**1. Лимиты стали per-tenant (были вшиты в код как демо-значения).** Колонка `businesses.limits` (JSON), `config.parseLimits(json, isDemo)` с наборами `DEMO_LIMITS` (20 сообщ/клиент, 300 глоб, 5 голос, 2 активные брони) и `LIVE_LIMITS` (80/2000/25/5). Подключено во ВСЕХ точках: `index.ts respondTurn`, `web.ts handleChat`, `tools.ts bookSlot` (лимит активных броней), промпт (`booking_limit`-фраза с числом из настроек). Аккаунтовые бюджеты (Whisper-нейроны, ElevenLabs-кредиты) остались глобальными — они общие для всех тенантов. Битый JSON никогда не блокирует салон (падает в дефолты).

**2. Онбординг салона за минуту (был ручной SQL).** `scripts/onboard.ts` (`npm run onboard salon.json [--apply]`): валидирует часы и кратность длительностей ДО записи, генерирует SQL с id относительно вставленной строки, печатает admin-токен (показывается один раз), ставит `is_demo=0`. Пример — `launch/salon-example.json`.

**3. Мониторинг (не было вообще).** `src/health.ts`: `healthCheck` (D1 жива, секреты на месте, Turnstile боевой, остаток ElevenLabs-кредитов/Whisper, демо-сид не поредел) — без единого секрета в ответе. `GET /api/health` → JSON (200/503). `scheduled()` после resetDemo зовёт `runHealthAlert`, который пингует владельца в Telegram ТОЛЬКО при проблеме (молчит, когда всё зелёное). `?alert=<WEBHOOK_SECRET>` — ручной триггер проверки канала алертов.

**Приёмка (живьём на деплое):**
- `tsc` 0; `vitest` — **68 тестов** (limits per-tenant + LIVE-салон держит 3 брони, где демо режет на 2).
- Онбординг: валидатор отверг длительность 25 при шаге 30; создан временный второй салон `isolation-test` в живой D1. **Изоляция подтверждена:** новый салон отдаёт свои услуги/часы (Бахыт, 09–14:30), demo-only `owner-feed` → 404, его admin-токен видит только «Тест-салон» (0 броней). **resetDemo НЕ тронул** бронь реального салона (пережила ночной сброс) — демо-тенант изолирован. Салон удалён (CASCADE подчистил всё).
- Health: `GET /api/health` → `status:ok`, 7 проверок зелёные. Симулировал остаток кредитов 300 → `warn`; `?alert=` → `alerted:true`, запрос `outcome:ok` без исключений = владельцу ушёл Telegram-пинг «🟡 …elevenlabs_credits: 300/10000». Тестовое состояние восстановлено, health снова зелёный.
